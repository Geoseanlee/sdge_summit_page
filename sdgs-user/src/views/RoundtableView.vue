<template>
  <div class="roundtable">
    ...
    <div class="roundtable-content">
      <div class="container">
        <!-- 会议详细介绍（动态加载） -->
        <section class="forum-intro-section" v-if="introContent">
          <h2 class="forum-intro-title">{{ introTitle }}</h2>
          <div class="forum-intro-content" v-html="introContent"></div>
        </section>
...
const fetchForumIntro = async () => {
  try {
    const res = await request.get('/forum-info/latest')
    if (res) {
      introTitle.value = res.title || ''
      introContent.value = res.content || ''
    }
  } catch (error) {
    console.error('获取论坛介绍失败', error)
  }
}
