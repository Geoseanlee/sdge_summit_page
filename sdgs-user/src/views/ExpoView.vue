<template>
    <div class="expo">
        <!-- 页面头部 - 只在主页面显示 -->
        <div v-if="$route.matched.length === 1" class="page-header">
            <div class="container">
                <h1>世博会</h1>
                <p>探索联合国17个可持续发展目标</p>
        ...
        <div v-else class="expo-content">
            <div class="container">
                ...
                <div class="search-section">
                    <el-input v-model="searchQuery" placeholder="搜索SDG目标..." prefix-icon="Search" size="large"
                        style="max-width: 400px" />
                </div>

                <!-- SDGs 卡片网格 -->
                <div class="sdg-grid">
                    <div v-for="sdg in filteredSDGs" :key="sdg.id" class="sdg-card"
                        :style="{ backgroundColor: sdg.color }" @click="openSDGDetail(sdg)">
        ...
        <!-- SDG 详情对话框 -->
        <el-dialog v-model="dialogVisible" :title="`SDG ${selectedSDG?.id}: ${selectedSDG?.title}`" width="80%"
            max-width="800px">
